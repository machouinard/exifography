...
if ( ! class_exists( "exifography" ) ) {
	class exifography {
		...
		protected $exif_options = 'exifography_options';
		...
		private function init() {
			...
			add_action( 'wp_ajax_insert_exif', array( $this, 'ajax_insert_exif' ) );
			add_action( 'wp_ajax_nopriv_insert_exif', array( $this, 'ajax_insert_exif' ) );
			...
			add_filter( 'image_send_to_editor', array( $this, 'add_data_attr_to_image' ), 10, 2 );
		}
		...
		function add_exif( $meta, $file, $sourceImageType ) {
			...
			if ( isset( $_REQUEST['post_id'] ) ) {
				...
				update_post_meta( $post_id, 'image_date', $meta['created_timestamp'] );
			}
...
&markers=color:blue%7Clabel:S%7C' . $geo_coords . '&sensor=false';
		...
		public function display_exif( $post_id = null, $display = null, $imgID = null ) {
		...
		function auto_insert( $content ) {
			...
			preg_match_all( '~<a[\s]+([^>]+)>?:.?!\<\/a\>*.<\/a>~', $content, $matches );
		...
		function admin_page() {
			...
			add_options_page( 'Exifography Options', 'Exifography', 'manage_options', 'exifography', array(
				$this,
				'options_page',
			) );
		}
		...
		function options_page() {
            ...
            <div>
                ...
                <form action="options.php" method="post" class="exifography">
                    ...
                    <p><input class="button-primary" name="Submit" type="submit"
		...
		function options_init() {
			...
			add_settings_section( 'default_display', __( 'Default EXIF', 'exifography' ), array(
				$this,
				'defaults',
			), 'plugin_options' );
			add_settings_section( 'auto_display', __( 'Auto insert into post', 'exifography' ), array(
				$this,
				'auto',
			), 'plugin_options' );
			add_settings_section( 'custom_html', __( 'Custom HTML', 'exifography' ), array(
				$this,
				'html',
			), 'plugin_options' );
			// exif fields settings inputs
			foreach ( $this->fields as $key => $value ) {
				add_settings_field( 'exif-field-' . $key, $value, array(
					$this,
					'default_fields',
				), 'plugin_options', 'default_display', $key );
			}
			// auto insert settings fields
			add_settings_field( 'auto_insert', __( 'Automatically display exif', 'exifography' ), array(
				$this,
				'auto_field',
			), 'plugin_options', 'auto_display' );
			// custom HTML settings fields
			foreach ( $this->html_options as $key => $value ) {
				add_settings_field( $key, $value, array(
					$this,
					'html_fields',
				), 'plugin_options', 'custom_html', $key );
			}
			add_settings_field( 'timestamp', __( 'Timestamp format', 'exifography' ), array(
				$this,
				'timestamp',
			), 'plugin_options', 'custom_html' );
			add_settings_field( 'item_label', __( 'Turn off item label', 'exifography' ), array(
				$this,
				'label',
			), 'plugin_options', 'custom_html' );
			add_settings_field( 'geo_link', __( 'Link GEO EXIF to Google Maps', 'exifography' ), array(
				$this,
				'geo_link',
			), 'plugin_options', 'custom_html' );
			add_settings_field( 'geo_img', __( 'Display map thumbnail instead of location coords', 'exifography' ), array(
				$this,
				'geo_img',
			), 'plugin_options', 'custom_html' );
			add_settings_field( 'geo_zoom', __( 'Map zoom (0 is the widest, 21 is close)', 'exifography' ), array(
				$this,
				'geo_zoom',
			), 'plugin_options', 'custom_html' );
			add_settings_field( 'geo_width', __( 'Map width', 'exifography' ), array(
				$this,
				'geo_width',
			), 'plugin_options', 'custom_html' );
			add_settings_field( 'geo_height', __( 'Map height', 'exifography' ), array(
				$this,
				'geo_height',
			), 'plugin_options', 'custom_html' );
		...
		function add_post_box() {
			...
			if ( function_exists( 'add_meta_box' ) ) {
				add_meta_box( 'exifography_add_meta', __( 'Add EXIF to post', 'exifography' ), array(
					$this,
					'edit_post_exif',
				), 'post', 'side', 'low' );
			}
		...
		//* Enqueue and localize JS for front-end
		function front_end_scripts() {

			wp_enqueue_script( 'add-exif', plugin_dir_url( __FILE__ ) . 'js/front.js', array( 'jquery' ), self::$version, true );

			$options = array(
				'ajaxurl'    => admin_url( 'admin-ajax.php' ),
				'exif_nonce' => wp_create_nonce( 'exifsecurity' ),
			);

			wp_localize_script( 'add-exif', 'exifOpts', $options );

		}

		/**
		 * Add EXIF data to images via AJAX
		 *
		 * @since 1.3.0
		 *
		 * @return void
		 */
		function ajax_insert_exif() {

			check_ajax_referer( 'exifsecurity', 'security' );
			...
			if ( isset( $_POST['image_id'] ) ) {
				...
				$output = $this->display_exif( $post_id, null, $image_id );
		...
		/**
		 * Add Post and Image IDs as data-attributes to images for JS
		 *
		 * @since 1.3.0
		 *
		 * @param $html
		 ...
		 * @return string
		 */
		function add_data_attr_to_image( $html, $id ) {
			...
			if ( $id ) {
				...
				foreach ( $images as $image ) {
					...
					$image->setAttribute( 'data-image_id', $id );
					$image->setAttribute( 'data-post_id', $parent );
...
function exifography_display_exif( $post_id = null, $fields = null, $imgID = null ) {

	if ( class_exists( 'exifography' ) ) {
		$exif = new exifography();

		return $exif->display_exif( $post_id, $fields, $imgID );
	}
}
